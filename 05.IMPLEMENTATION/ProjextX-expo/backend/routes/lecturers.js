...
router.get('/:id', async (req, res) => {
  try {
    ...
    if (req.params.id.match(/^[0-9a-fA-F]{24}$/)) {
      lecturer = await Lecturer.findById(req.params.id).select('-password');
    }
    
    // If not found by _id, try to find by lecturerId
    if (!lecturer) {
      lecturer = await Lecturer.findOne({ lecturerId: req.params.id }).select('-password');
    }
...
router.put('/:id', async (req, res) => {
  try {
    ...
    // If not found by _id, try to find by lecturerId
    if (!lecturer) {
      lecturer = await Lecturer.findOne({ lecturerId: req.params.id });
    }
